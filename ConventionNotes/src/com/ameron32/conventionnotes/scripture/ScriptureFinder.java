...
//	  }
	...
	private InputStream openBibleFile() throws ChapterNotFoundError{		
	...
	private  String getFileName()throws ScriptureNotFoundError{
		String fileName = "";
		...
		if (chapter == 1){
			if (val>99){
				...
				fileName = 	"0"+String.valueOf(val)+"_"+bookAbbrev+"-1.xhtml";
		}else
		{
			if (val>99){
				fileName= String.valueOf(val)+"_"+bookAbbrev+"-1-split"+String.valueOf(chapter)+".xhtml";
			}else
				fileName = "0"+String.valueOf(val)+"_"+bookAbbrev+"-1-split"+String.valueOf(chapter)+".xhtml";
		}
		return fileName;
	}

	private  String getBookAbbrev(String bookName) throws BookNotFoundError{
	...
	private  String getVerse(int v) throws ScriptureNotFoundError{		
		...
		for (int i=0; i<verseArray.size(); i++){
			if (verseArray.get(i).contains(start)){				
				...
				while (!verseArray.get(i).contains(end) && !verseArray.get(i).contains(endFileMarker)){
					included.add(i);
					i++;
				}				
				i = verseArray.size();				
...
//	    Testing.endTest("loop");
	...
	private String getVerseB(int v)
		      throws ScriptureNotFoundError {
		    Testing.startTest("getVerseB");
		    
		    final String startVerseMark = getStartVerseMarkerString(v);
		    final String endVerseMark = getEndVerseMarkerString(v);
