...
public class ProgramList {
  ...
  public static List<ProgramEvent> events          = new ArrayList<>();
  ...
  static {
    ...
    for (int i = 0; i < times.size() - 1; i=i+2) {
      ProgramEvent event = events.get(i/2);
  ...
  private static void addToProgram(ProgramEvent event) {
    
    boolean added = events.add(event);
    if (event instanceof Talk) {
      Talk talk = (Talk) event;
      talk.setTalkNumber(talks.size());
      talks.add(talk);
    }
  }
}
